      ...
      module LoginAndPassword
        def self.included(base) #:nodoc:
          ...
          base.class_eval do
            ...
            validates_presence_of     :password,                   :if => "needs_password? && password_not_saved?", :message => :"password.blank"
            validates_presence_of     :password_confirmation,      :if => "needs_password? && password_not_saved?", :message => :"password_confirmation.blank"
            validates_length_of       :password, :within => 4..40, :if => "needs_password? && password_not_saved?", :message => :"password.length"
            validates_confirmation_of :password,                   :if => "needs_password? && password_not_saved?", :message => :"password.confirmation"
            # prevents a user from submitting a crafted form that bypasses activation
            # anything else you want your user to change should be added here.
            attr_accessible :password, :password_confirmation

            if agent_options[:login_and_password][:login]
              validates_presence_of     :login, :message => :"login.blank"
              validates_length_of       :login, :within => 1..40, :message => :"login.length"
            ...
            if agent_options[:login_and_password][:email]
              ...
              validates_length_of       :email, :within => 5..100, :message => :"email.length"
